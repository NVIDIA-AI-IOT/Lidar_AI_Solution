...
def bev_pool(feats, coords, B, D, H, W, export= False):
    ...
    if export:
        x = QuickCumsumCuda.apply(feats, coords[:, :4].contiguous(), ranks, B, D, H, W)
    else:
    ...
    if export:
        ...
        return x, torch.stack([interval_starts, interval_starts+interval_lengths, interval_output_position], dim=1).int(), coords[:, 4].int()
    else:
        return x
